...
<template>
    <div v-if="component_show" class="w-full grid grid-cols-12 p-2">
        ...
        <span class="text-sm font-bold col-span-2 self-center text-center text_ani" :class="text_class"> {{ time }}次</span>
    </div>
</template>
...
.progressive_bar_ani {
    animation: p_bar_ani 1s cubic-bezier(.44,1.02,.58,.92) forwards;
}

.text_ani {
    animation: t_ani 1s cubic-bezier(.44,1.02,.58,.92) forwards;
}
